...
export const POST: APIRoute = async ({ request }) => {
  ...
  <h2 style="font-size:16px;font-weight:700;margin:0 0 8px">
    <a href="${postUrl}" style="color:#9333ea;text-decoration:none">${escapeHtml(post.title)}</a>
  </h2>
  ${post.description ? `<p style="color:#6b7280;font-size:13px;line-height:1.5;margin:0 0 12px">${escapeHtml(post.description)}</p>` : ""}
  <a href="${postUrl}" style="display:inline-block;background:#9333ea;color:#fff;font-size:13px;padding:6px 14px;border-radius:6px;text-decoration:none">阅读全文 →</a>
</div>`;
		}).join("");
...
<body style="font-family:-apple-system,sans-serif;background:#f5f5f5;margin:0;padding:24px">
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08)">
    <div style="background:linear-gradient(135deg,#7e22ce,#9333ea);padding:28px 24px;text-align:center">
